...
class Db:
    ...
    def __init__(self, uri, database_name):
        ...
        self.live_cfg = self.db.live_config   # ← per-user live forward config
...
# ================= LIVE FORWARD CONFIG (per-user) ================= #

    _DEFAULT_LIVE_FILTERS = {
    ...
    async def get_live_config(self, user_id: int) -> dict:
        default = {
            'user_id': int(user_id),
            'destination': None,
            'destination_title': '',
            'sources': [],
            'is_active': False,
            'filters': self._DEFAULT_LIVE_FILTERS.copy(),
        }
        doc = await self.live_cfg.find_one({'user_id': int(user_id)})
        if not doc:
            return default
    ...
    async def update_live_config(self, user_id: int, updates: dict):
        await self.live_cfg.update_one(
            {'user_id': int(user_id)},
            {'$set': updates},
            upsert=True
        )

    async def get_all_active_live_configs(self):
        return self.live_cfg.find({'is_active': True})

    async def live_forward_count(self) -> int:
        return await self.live_cfg.count_documents({'is_active': True})
